 ...
 *   `FsError::InvalidUtf8` で失敗するため。代わりに読まない監視だけを張る
 *   `open_binary_document` を通すので、監視・削除追従・reload 復元(要件#10)は
 *   テキスト文書と同等に効く(要件#22。当初は対象外という割り切りだった)。
 *   SVG は UTF-8 XML なので従来のテキスト経路のまま
